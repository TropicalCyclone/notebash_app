...
import 'package:notebash_app/components/book_actions.dart';
import 'package:notebash_app/components/book_item.dart';
...
class BooksPage extends StatefulWidget {
  ...
  const BooksPage({super.key, required this.userId, required db}) : _db = db;
...
class _BooksPageState extends State<BooksPage> {
  ...
  void initState() {
    super.initState();
    _books = [];
    _service = BookService(db: widget._db);
  }

  Future<void> _load() async {
    var books = await _service.getByUserId(widget.userId);
    _books = books;
  }
  ...
  Future<void> _exportBooks() async {
    ...
    if (result.success) {
      if (mounted) {
        showSnackBar(
            context, 'File exported successfully to $folder\\books.json');
      }
    } else {
  ...
  Future<void> onImport(String contents) async {
    ...
    if (result.success) {
      await _load();
      setState(() {});
      if (mounted) showSnackBar(context, 'Books imported successfully');
    } else {
      if (mounted) showSnackBar(context, result.message!, error: true);
    }
  }
  ...
  void _showOptions(BuildContext context) {
    showModalBottomSheet<dynamic>(
      ...
      builder: (context) => BookActions(
        bookEntryPage: BookEntryPage(
        ...
        if (snapshot.connectionState == ConnectionState.waiting) {
          ...
          return Scaffold(
              appBar: AppBar(
                ...
                actions: [
                  IconButton(
                    onPressed: _books.isEmpty ? null : () => _exportBooks(),
